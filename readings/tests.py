...
class ReadingsTests(TestCase):
    ...
    def test_book_reading_redirect_if_not_logged_in(self):
        ...
        self.assertRedirects(
            response, '/accounts/login/?next=' + self.book_url)
    ...
    def test_successful_booking(self):
        ...
        self.assertTrue(
            Booking.objects.filter(
                user=self.user, reading_type='tarot').exists())
    ...
    def test_booking_conflict(self):
        ...
        self.assertTrue(
            any("overlaps" in msg.message.lower() for msg in messages))
    ...
    def test_get_booked_times_api(self):
        ...
        response = self.client.get(
            self.get_times_url, {'date': date.today(
                ).strftime('%Y-%m-%d'), 'reading_type': 'tarot'})
        self.assertEqual(response.status_code, 200)
